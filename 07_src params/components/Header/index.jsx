...
export default class Header extends Component {
    ...
    search = () =>{
        ...
        PubSub.publish("ceshi",{name:'tom',age:18})
        ...
        PubSub.publish("ceshi",{isFrist:false,isLoad:true})

        //切记在配置代理了之后一定需要添加相应的路径
        axios.get(`http://localhost:3000/api1/search/users?q=${keyWord}`).then(
            success => {
                // this.props.updateAppState({Git:success.data.items,isLoad:false});
                PubSub.publish("ceshi",{Git:success.data.items,isLoad:false})
            },
            error =>    {
                // this.props.updateAppState({isError:error.message,isLoad:false});
                PubSub.publish("ceshi",{isError:error.message,isLoad:false})
            }
        )
    }
